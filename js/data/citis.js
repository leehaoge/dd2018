define(['utils/pinyin_dict_firstletter', 'utils/pinyinUtil'], function (dict, pyUtil) {
    'use strict';
    ...
    var PYs = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', pyIndexed = {};

    for (var idx in PYs) {
        var c = PYs.charAt(idx);
        pyIndexed[c] = [];
    }
    ...
    for (var p in pc) {
        var cofp = pc[p];
        for (var i in cofp) {
            var city = cofp[i], py = window.pinyinUtil.getFirstLetter(city);
            if (py) {
                var fl = py.charAt(0);
                pyIndexed[fl].push(city);
            }
        }
